crit_vals = load("crit_Fr_tau0_theta_water_no_pe.txt");
...
C = viridis(n_pts1+1);
coeff_vals = [0,5,10,20,25]; %[1e-5 1e-4 1e-3]; % [1e-6 5e-6 1e-5 5e-5 1e-4];
...
% ylabel("Critical $Fr$")
% xlim([theta_start,theta_stop])
title("Fluid -- water") %,, , $\tau_0 = 0$Pa, $\alpha =10^{-5}$ Pa$^{-1}$, $d =10^{-4}$m

exp_graph(gcf,"crit_Fr_tau0_water_no_pe.pdf")