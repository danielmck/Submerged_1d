...
%     rho_f = 1;
    ...
    if ~exist('master_y','var')
        ...
        if strcmp(wave_type,'full')
            ...
            master_pea = master_y(9,1) + (rho/rho_f*chi-1).*master_y(4,1);
        end
    else
    ...
    pea_list = linspace(master_pea,master_pea-pea_alter,n_steps);
    A_list = linspace(final_A,target_A,n_steps);
    
    [xi_final,y_final] = run_bvp_step(Fr_list, theta_list, nu_list, alpha_list, d_list, pea_list, A_list, master_xi, master_y);
%     [xi_nope,y_nope] = viscous_Iv_bvp_from_master(true,[Fr_eq,theta,lambda,nu]);
    ...
    if ~custom_init
        ...
        filename = "theta_9_5.txt";
        write_record("Results/wave_record.csv",filename,{"full","water",Fr_eq,theta,target_A,nu,alpha,d})
        save("Results/"+filename,"out_vec","-ascii")
%         plot(xi_out,u)

        hold on
        plot(lambda_out*xi_final,u)
...
%     plot(xi_nope,y_nope(3,:))
    ...
    function [xi_out, y_out] = run_bvp_step(Fr_vals, theta_vals, nu_vals, alpha_vals, d_vals, pea_vals, A_vals, xi_in, y_in, tol,counter)
...
%                 xi_run = horzcat(linspace(0,3*lambda_vals(i)/4,70),linspace(3*lambda_vals(i)/4, lambda_vals(i),70));
        ...
        function [xi_return,y_return,non_pe] = recurse_manage()
            ...
            [xi_return,y_return] = run_bvp_step(linspace(Fr_vals(i-1),Fr_in,3)...
                        ...
                        ,alpha_vals(i),3),linspace(d_vals(i-1),d_vals(i),3),linspace(pea_vals(i-1),pea_vals(i),3)...
                        ,linspace(A_vals(i-1),A_vals(i),3),xi_in, y_in, tol,counter+1);
            non_pe = false;
            if counter == 1
        ...
        function resid = bc_vals(ya,yb)
            pe_a = ya(9) + (rho_dl*g_dl*cosd(theta_in)*chi-1).*ya(4);
            resid = [ya(4)-yb(4), ya(5), yb(5), ya(6), yb(6)-1, ya(7), yb(7)-A_in, ya(8)-yb(8), ya(9)-yb(9)]; %
        end
    end      
