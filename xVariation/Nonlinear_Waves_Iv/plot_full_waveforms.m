...
% theta, lambda, alpha and d.
    ...
    var_list = [[0.8,1,1.2];[10,12,14];[8,12,16];[1e-5,1e-4,5e-4];[1e-6,1e-5,1e-4];[1e-5,1e-4,1e-3]];
    ...
    for k = 3:n_var
        ...
        for i = 1:n_vals
            ...
            [xi_temp,y_out] = bvp_full_from_master(param_in,master_y,master_xi,master_param);
            xi_out{i,1} = xi_temp;
        ...
        for m = 1:n_var
            if m~=k
                if abs(params(m))>1e-2
                    val_str = num2str(params(m));
                else
                    val_str = num2str(params(m),'%.2e');
                end
        ...
        for j = 1:5
            ...
            hold on
            for l=1:n_vals
                plot(xi_out{l,1},plot_vec{l,:}, "DisplayName","$"+var_names(k)+"="+num2str(var_list(k,l))+"$","color",C(l,:))
            end
            xlabel("$\xi$")
            ylabel("$"+ax_title(j)+"$")
