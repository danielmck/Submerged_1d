...
%         [Fr_eq, ~] = crit_Iv_tau0_h(theta, rho_p, rho_f, eta_f, h0, tau0);
    ...
    if ~exist('master_y','var')
        ...
        master_nu_coeff = record.nu(in_table);
...
%         if strcmp(wave_type,'full')
        ...
        master_rf = 1;
%         end
        master_params = [master_Fr,master_theta,master_lambda,master_nu_coeff,master_alpha,master_d,master_tau0];
    else   
        master_temp = num2cell(master_params);
        if (size(param_cell,2) == 7)
            [master_Fr,master_theta,master_lambda,master_nu_coeff,master_alpha,master_d,master_tau0] = master_temp{:};
            rel_flux = 1;
            pres_h = 0;
        elseif (size(param_cell,2) == 9)
            [master_Fr,master_theta,master_lambda,master_nu_coeff,master_alpha,master_d,master_tau0,master_rf,master_pres_h] = master_temp{:};
        end
    end
...
%     end
    ...
    nu_ratio = max(nu_coeff/master_nu_coeff,master_nu_coeff/nu_coeff);
    ...
    ratio_sum = Fr_ratio-1+nu_ratio-1+30*(theta_ratio-1)+d_ratio-1+alpha_ratio-1+tau0_ratio+80*(lambda_ratio-1)+rf_ratio;
    if ratio_sum > 1e-6 %
...
%     end
    ...
    nu_list = linspace(master_nu_coeff,nu_coeff,n_steps);
...
%                 p_tot_grad_dl = p_tot/p_scale*z_scale;
                ...
                rho_eq_dl = rho_p_dl*phi_eq+rho_f_dl*(1-phi_eq);
                chi_eq = (rho_f_dl+3*rho_eq_dl)/(4*rho_eq_dl);
...
%             dndxi = 1/(2*h)*n^2 + h^(3/2)/Fr_in^2/nu_dl/Q1*n_coeff*(n-n_eq);
            dmdxi = rho_dl*h/lambda_in/rho_eq_dl*u^(1-pres_h);
