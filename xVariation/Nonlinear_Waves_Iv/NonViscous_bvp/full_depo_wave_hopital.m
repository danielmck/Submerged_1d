function [xi_final,y_final] = full_depo_wave_hopital(specify_param,params,provide_init,master_xi,master_y,master_params)
...
% xi, master_y as the initial y and master_params as the initial parameters
    ...
    if (pres_h ~= master_pres_h)
        ...
        u_flux = (master_y(1,1) - master_y(4,:)./master_y(5,:)).^(1-pres_h);
        for j = 1:size(master_xi,2)
            m_init(j) = m_val;
            if j < size(master_xi,2)
                scale = (master_xi(j)>1)*(master_lambda-master_h_crit)+(master_xi(j)<1)*master_h_crit;
                m_val = m_val + scale/master_lambda* (master_y(5,j)*u_flux(j)^(1-pres_h)+master_y(5,j+1)*u_flux(j+1)^(1-pres_h))/2*(master_xi(j+1)-master_xi(j));
            end
        end
        master_y = vertcat(master_y(1:5,:),m_init,master_y(7:end,:));
        master_rf = m_val;
    end
...
%     master_y = master_y(4:8,:);
    ...
    if ~specify_param
        ...
        write_record("Results/wave_record.csv",filename,{"full_no_vis","water",Fr_eq,theta,alpha,d,tau0})
    end
    ...
    function [xi_in, y_in] = run_bvp_step(Fr_vals, theta_vals, tau0_vals, wlen_vals, alpha_vals, d_vals, rf_vals, xi_in, y_in, tol,counter)
        ...
        if counter > 10
            ...
            fail_name = "lambda_limit_better.txt";
            save("Results/"+fail_name,"out_vec","-ascii")
            write_record("Results/wave_record.csv",fail_name,{"full","water",Fr_vals(1),theta_vals(1),alpha_vals(1),d_vals(1),tau0_vals(1)})
...
%             end
            ...
            try
                if stat_reg
                    ...
                    solInit1_np=bvpinit(xi_in,@bvp_guess);
                    opts = bvpset('RelTol',tol,'NMax',2000*counter);
                    solOut = bvp4c(@(x,y,r) viscous_syst_np(x,y,r),@bc_vals_np,solInit1_np,opts);
...
%                     solOut = bvp4c(@(x,y,r,p) viscous_syst(x,y,r,p),@bc_vals,solInit1,opts);
        ...
        function dydxi = viscous_syst(xi,y,p)
            ...
            if denom<denom_eps
                pp_coeff = -mu_Iv_fn(Iv)/(p_tot_grad_dl*h)+p_p/(p_tot_grad_dl*h)*dmudIv_fn(Iv)*3*eta_f_dl*abs(u)/h/p_p^2;
                pb_coeff = P*2/beta_dl-pp_coeff;
                h_coeff = p_p/(p_tot_grad_dl*h^2)*mu_Iv_fn(Iv)+p_p/(p_tot_grad_dl*h)*dmudIv_fn(Iv)*3*eta_f_dl*abs(u)/h^2/p_p+tau0_dl*rho_f/rho/h^2*-P*2/beta_dl+pp_coeff*rho*g*cosd(theta);

                h_quad_a = 3*h^2/Fr^2;
                h_quad_b = -2*Q1*dQdxi-h^3/Fr^2*(g_dl*cosd(theta)*rho_dl*chi*pb_coeff-h_coeff+p_p/(p_tot_grad_dl*h)*dmudIv_fn(Iv)*Iv/u*u_w/h^2)-3*h^2/Fr^2*(fb_val+P*D*h);
                h_quad_c = h^3/Fr^2*(dy7dxi*pb_coeff-p_p/(p_tot_grad_dl*h)*dmudIv_fn(Iv)*Iv/u*dQdxi);
                h_quad_roots = roots([h_quad_a,h_quad_b,h_quad_c]);
                dydxi(5) = min(h_quad_roots(h_quad_roots>0));
            end 
            dydxi = dydxi';
        end
        ...
        function resid = bc_vals(ya,yb,p)
            if set_h_min
                ...
                h_stop = (-ya(2) + sqrt(ya(2).^2+8*ya(1)^2./(ya(2)/Fr_in^2)))/2;
                len_resid = [p(2)-lambda_in, yb(2)-h_stop];
            end
            resid = [ya(1)-yb(1), ya(3), yb(3)-rf_in,...
                (ya(4)-yb(4)), (ya(5)-yb(5)), len_resid];
        end
            
        function dydxi = viscous_syst_static(xi,y,p)
            % The system from the viroulet paper with a different friction
            % law. Need to solve for u_w and Q1 too.
            u_w = p(1);
            lambda = p(2);
