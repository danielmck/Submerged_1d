function compare_all_angles
    ...
    DA_ave = load("Iverson_DA/DA_Results/DA_pe_base_angles_v2.txt");
    ...
    Ive_DA_diff = (DA_ave(:,2:end))./Iverson_ave*100;
...
%     SetPaperSize(12,10);
    x = linspace(0.3,7,68);
    y = linspace(50,500,46);
    [Y,X] = meshgrid(y,x);
    SetPaperSize(12,10);
    hold on
    [C,h] = contourf(Y,X,log2(Rau_Ive_diff(5:50,3:end)'/100),100,"HandleVisibility",'off');
%     plot(10*Ive_depo_start,linspace(0.1,7,70),'black','LineWidth',1.5,'DisplayName',"Iverson Deposition");
%     plot(10*Rau_depo_start,linspace(0,7,71),'red','LineWidth',1.5,'DisplayName',"Rauter Deposition");
...
%     legend('Location', "southeast",'UserData', 12)
    ...
    c = colorbar;
    c.Label.String = "Log Base 2 of Ratio of Basal Excess Pressure";
%     caxis([0,200])
    ylabel("$\theta$")
    xlabel("$t$")
    title("Log of Ratio between Basal $p_e$ in the Depth Averaged and Full Model")
    ax = gca;
    ax.XAxis.Exponent = 0;
%     xlim([50,15000])
    PrintFig('Ive_da_pe_early_log_comp')
end