...
%     subplot(1,2,1);
    ...
    plot_names = ["$\mu(I)$","$\mu(I_v)$"];
    for j=linspace(1,nfig,nfig)
        k = t1(i)+(j-1)*5;
        hold on
        t_val = (k-1)*t_step(i);
        plot(plot_vec(1:end,k),z_pe{i,1}(1:end),'DisplayName',"t= "+num2str(10*t_val))
%         plot(crit_pe,z_pe{i,1}(1:end),'DisplayName',"$\phi(I_v)$");
        legend('Location', "northeast",'UserData', 10);
        ylabel("$z/h$");
        xlabel('$p_p$');
        box on
...
% % %     plot((-buoyancy(i)+crit_grad)*(1-z_u{i,1}),z_u{i,1},"k--",'DisplayName',"Critical $p_e$")
    legend('Location', "southwest",'UserData', 12);
%     ylabel("z/h");
%     xlabel('$\frac{\partial p_e}{\partial t}$');
% % % 
% % %     
    title('Particle Pressure Minimum vs Analytic Approximation')
%      SetPaperSize(10,10);
...
%      xtickformat('%5.1e');
% %     hold off
    pdfname = 'Initial_pp_evo_vs_approx';    
...
% u_ave1 = depth_average(u_p{1,1},N(1),5001);
